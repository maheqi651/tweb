...
function updateservice(servicedid){
	window.location.href="updateserviceadmin.html?serviceid="+servicedid;
}
function spyj(str)
{
  $("#reply").val(str);
...
function dotable(status){
	...
	if(status==0)
		{
           htmlstr="<tr><th>服务名称 </th><th>请求方法 </th><th> 帮助链接</th><th>注册单位 </th><th>注册人 </th><th>注册时间</th><th>操作</th></tr>";
		   ...
		   htmlstr+="<tr><th> 服务名称 </th> <th> 请求方法 </th> <th width='10%'> 注册人</th>  <th> 注册时间</th> <th width='10%'> 审核人</th> <th> 审核时间 </th>";
        ...
        init: function () {
            ...
            var oTable = $('#expandabledatatable').dataTable({
                ...
                { "bSortable": false, "aTargets": [0] },
				{
                    ...
                    "aTargets": [6],
                    "mRender": function (data, type, full) {
                        return '<div class="btn-group btn-group-xs" role="group"><a  class="btn btn-primary btn-xs"  onclick="appli(\''+full.id+'\')"><i class="icon-wrench icon-white" ></i>详情</a> <a   class="btn btn-warning btn-xs edit" onclick="updateservice(\''+full.id+'\')"><i class="fa fa-edit"></i> 修改</a></div>' ;
                    }
                }],
                "aaSorting": [[5, 'desc']],
                "aLengthMenu": [
    ...
    return {
        init: function () {
          ...
          *  <th> 审核人</th> <th> 审核时间 </th>";
            ...
            var oTable = $('#expandabledatatable').dataTable({
                "sDom": "Tflt<'row DTTTFooter'<'col-sm-6'i><'col-sm-6'p>>",
                "sAjaxSource": "fg/findFwSq.do?status="+$("#statusid").val(), //ajax请求地址
                "aoColumns" : [ {
                    "bVisible" : true,
                    "data" : "fwname",
                    "aTargets" : [0]
                ...
                }, 
                {
                    ...
                    "mRender": function (data, type, full) {
                    	 ...
                    	 if(full.applyuser==""||full.applyuser==null)
                    		 {
                    		 return '';
                    		 }else if(full.applyuser.length>10){
                    			 return full.applyuser.substring(0,10)+'...';
                    		 }else{
                    			 return full.applyuser;
                    		 }
                ...
                { "bSortable": false, "aTargets": [0] },
				{
                    ...
                    "mRender": function (data, type, full) {
                    	...
                    	/*if(statusid=="1")
                    		{
                    		  return '<div class="btn-group btn-group-xs" role="group"><a  class="btn btn-success btn-xs"   ></i>通过</a>  </div>'  ;
                    		}else{
                    		  return '<div class="btn-group btn-group-xs" role="group"><a  class="btn btn-danger btn-xs"   ></i>拒绝</a> </div>'  ;
                    		}*/
                    	if(statusid=="1")
                		{
                		  return '通过'  ;
                    ...
                    "mRender": function (data, type, full) {
                    	...
                    	if(statusid=="1")
                		{
                    		 return '<div class="btn-group btn-group-xs" role="group"><a  class="btn btn-info btn-xs"  onclick="spyj(\''+full.reply+'\')" ></i>审核意见</a><a  class="btn btn-primary btn-xs"  onclick="appli(\''+full.id+'\')"><i class="icon-wrench icon-white" ></i>详情</a> <a   class="btn btn-warning btn-xs edit" onclick="updateservice(\''+full.id+'\')">  修改</a></div>' ;
                		}else{
