	...
	if(status==0)
		{
		   ...
		   htmlstr+="<tr><th width='15%'> 服务名称 </th> <th width='10%'> 应用名称 </th> <th width='15%'> 应用描述 </th>   <th width='15%'> 申请时间</th> <th width='8%'> 申请人 </th>";
		   htmlstr+="<th> 审批时间</th><th width='8%'> 审批人 </th><th width='8%'>审核状态</th><th width='10%'>操作</th></tr>";
		   $("#tablethread").html(htmlstr);
...
var InitiateExpandableDataTable1 = function () {
    return {
        init: function () {
            ...
            var oTable = $('#expandabledatatable').dataTable({
                "sDom": "Tflt<'row DTTTFooter'<'col-sm-6'i><'col-sm-6'p>>",
                "sAjaxSource": "fg/findServiceApply.do?flag=0&status="+$("#statusid").val(), //ajax请求地址
                "aoColumns" : [ {
                    "bVisible" : true,
                    "data" : "name",
                    "aTargets" : [0]
                ...
                { "bSortable": false, "aTargets": [0] },
				{
                    ...
                    "mRender": function (data, type, full) {
                    	...
                    	if(statusid=="1")
                		{
                			...
                			return '<div class="btn-group btn-group-xs" role="group"><a  class="btn btn-info btn-xs"  onclick="spyj(\''+full.reply+'\')" ></i>查看意见</a><a  class="btn btn-primary btn-xs"  onclick="appli(\''+full.name+'\',\''+full.description+'\',\''+full.servicecode+'\',\''+full.aid+'\',\''+full.methodname+'\')"><i class="icon-wrench icon-white"></i>详情</a></div>'  ;
                		}
