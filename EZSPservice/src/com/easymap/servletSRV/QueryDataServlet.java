...
import com.easymap.base.timeout.TimeOutAsyncListener;
...
public class QueryDataServlet   implements Action {
...
/*						if (order.endsWith(","))
						...
						istotal=false;
					}else if(eIName.getTextTrim().equals("TimeOut"))
					{//超时限制
						isTimeOut=true;
						timeOutNum = 1000*Long.parseLong(e.element("Value").element("Data")
								.getTextTrim());
					}
				}
			}
			...
			if(request.getAttribute(Tools.FLAG)!=null)   //提供运维验证标记;
			{
			 ...
			 System.out.println(themeCode+"++++"+senderID+"++++"+dataObjectCode);
			   if(isTimeOut)
			   {
				    ...
				    asyncCtx.addListener(new TimeOutAsyncListener());
				    asyncCtx.setTimeout(timeOutNum);
				    ThreadPoolExecutor executor = (ThreadPoolExecutor) request
			                .getServletContext().getAttribute("executor");
				    
			        executor.execute(new com.easymap.base.timeout.AsyncRequestProcessor(asyncCtx, senderID, 
			        		dataObjectCode, themeCode,methodName,
