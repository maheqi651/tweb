	...
	$.ajax({
		...
		success : function(restext) {
			...
			{   
				...
				for(var oo in results3)
				{
					...
					for(var ooo in results4){
						var results5=results4[ooo];
						if(results5.ID2==$("#idcard").val()){
							content+="<tr style='color:red'  >";
						}else{
							content+="<tr >";
						}
						if(countstemp==0)
						{
							content+="<td VLIGN='MIDDLE' rowspan='"+cols+"' >"+oo+"</td>";
						}
						countstemp++;
						content+="<td>"+results5.XM2+"</td>";
						content+="<td>"+results5.ID2+"</td>";
						content+="<td>"+results5.XB2+"</td>";
						content+="<td>"+dotime(results5.TIME)+"</td>";
						content+="<td>"+dotime(results5.OUTTIME)+"</td>";
						content+="<td><div class='btn-group'> <button class='btn  btn-xs btn-default' onclick='desp(\"入所原因\",\""+results5.DESP+"\")'>入所原因</button></div></td>";
						content+="</tr>";
					}
					objlist1[count]=content;
					count++;
			...
			if(parseInt(restext.totalcount)>0)
			{
				if(types=='02')
				{
					$("#biaoqian2").html("<span class='glyphicon glyphicon-lock'></span>同监室<span id='biaoqian1' class='label label-warning'>"+restext.totalcount+"</span>");
				}
			}
			$.extend(param, {page : 1});
			pagecontent(types);
