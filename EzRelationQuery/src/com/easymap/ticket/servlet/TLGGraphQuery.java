...
public class TLGGraphQuery extends HttpServlet{
	...
	public  void insertLOG(HttpServletRequest request)
	{
		...
		try {
			...
			try {
					logManager.setLog("440100120005", orgId, orgId, orgcode, new Date(System.currentTimeMillis()), "可视化情报分析","可视化情报分析关系分析图查询接口",  ip, "1", "查询身份证号为"+str+"的关系分析图数据");
				} catch (Exception e) {
				e.printStackTrace();
			}
		} catch (Exception e) {
	...
	throws Exception {
		...
		// JSONObject parm=JSONObject.fromObject(jsonstr);
		 ...
		 String rrtype =null;
		  ...
		  {
			  deap=Integer.parseInt(request.getParameter("deap"));
		  }
		  if(request.getParameter("idcard")!=null)
