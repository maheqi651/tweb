...
public class ProxWBRequest extends HttpServlet{
	...
	throws Exception {
	   ...
	   //  System.out.println("---------------"+counts);
	    // String[] strss=new String[6];
		 ...
		 int tcs=1;
		 if(jomain.get("tcs")!=null)
		 {
			 String stc=(String)jomain.get("tcs");
			  if(!stc.trim().equals(""))
				 {
				  tcs= Integer.parseInt(stc);
				 }
		 }
		 ...
		 for(Map.Entry<String,JSONObject> s:list)
		 {
			// System.out.print("-----111---------"+s.getValue().toString()); 
			 ...
			 { 
				 if(s.getValue().getInt("count")>=tcs)
				 {
