...
function searchTZSInfo(types){
   ...
   ispass=0;
	...
	requestUHB.starttime=$("#startdate").val();
	requestUHB.endtime=$("#enddate").val();
	requestUHB.hbaseTable="SJZ_DB_TEST8";
	requestUHB.qasType=qtype;
	requestUHB.params=new Object();
	requestUHB.count=$("#sjjg").val();
	requestUHB.params.rowkey="";
		...
		success : function(restext) {
           ...
           // alert(results2);
			...
			if(parseInt(restext.totalcount)>0)
    	    	{
			if(types=='05'){
	    		  $("#biaoqian5").html("<span class='glyphicon glyphicon-globe'></span>同上网<span id='biaoqian1' class='label label-warning'>"+parseInt(restext.totalcount)+"</span>");
			}}
			   $.extend(param, {page : 1});
		    	 pagecontent(types);
		...
		success : function(restext) {
           ...
           // alert(results2);
			...
			if(parseInt(restext.totalcount)>0)
    	    	{
			if(types=='04'){
	    		  $("#biaoqian4").html("<span class='glyphicon glyphicon-home'></span>同住宿<span id='biaoqian1' class='label label-warning'>"+parseInt(restext.totalcount)+"</span>");
			}}
			   $.extend(param, {page : 1});
		    	 pagecontent(types);
